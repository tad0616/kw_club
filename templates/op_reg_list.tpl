...
<div align="center">
    ...
    <select name="review" onChange="location.href='register.php?year=<{$smarty.session.club_year}>&review='+this.value;">       
            ...
            <{if $data.reg_sn == $reg_sn}>
                <tr id="tr_<{$data.class_id}>" bgcolor="#00FFCC">
                    ...
                    <form class="form-horizontal" name="regform" id="regform" action="<{$action}>" method="post" onsubmit="return xoopsFormValidate_regform();" enctype = "multipart/form-data">
                        ...
                        <td>                          
                            <label class="radio-inline"><input type='radio' name='reg_isreg'  title='正取' value='0' <{if $data.reg_isreg=='正取'}>checked<{/if}> />正取&nbsp;</label>
                            <label class="radio-inline"><input type='radio' name='reg_isreg'  title='後補' value='1' <{if $data.reg_isreg!='正取'}>checked<{/if}>>後補&nbsp;</label>
                        </td>
                        <td>                          
                            <label class="radio-inline"><input type='radio' name='reg_isfee'  title='未繳' value='0' <{if $data.reg_isfee!='1'}>checked<{/if}>>未繳&nbsp;</label>
                            <label class="radio-inline"><input type='radio' name='reg_isfee'  title='已繳' value='1' <{if $data.reg_isfee=='1'}>checked<{/if}>>已繳&nbsp;</label>
                      </td>
