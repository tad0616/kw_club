...
<div class="row">
    ...
    <div class="col-sm-9">
        <{if $class_year!=$year}>
            <span style='color: red'><{$class_year}></span>
        <{else}>
            <span style='color: blue'><{$class_year}></span>
        <{/if}>
    </div>
</div>
...
<div class="text-center">
    ...
    <{if $smarty.session.isclubAdmin || $uid == $class_uid }>
        ...
        <a href="<{$xoops_url}>/modules/kw_club/club.php&class_id=<{$class_id}>" class="btn btn-warning"><{$smarty.const._TAD_EDIT}></a>
        <a href="<{$xoops_url}>/modules/kw_club/club.php" class="btn btn-primary"><{$smarty.const._TAD_ADD}></a>
    <{/if}>
...
<{if $smarty.session.isclubAdmin || $uid == $class_uid }>
    <h2><span style="color:green"><{$class_title}></span>社團報名列表<small>（共 <{$class_regnum}> 筆報名資料）</small></h2>
